		...
		Spark.post("/chartData/", new Route() {
			...
			public Object handle( Request request, Response response ){
				try {
					...
					// First we attempt to extract the JWT from the header
					if (request.headers().contains("Authorization")){
						// TODO: Check why Spark is not allowing me access to read the header for Authorization, even though it exists.
					}
					else {
						response.status(401);
